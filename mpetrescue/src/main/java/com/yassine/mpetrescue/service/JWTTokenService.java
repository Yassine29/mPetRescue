...
final class JWTTokenService implements TokenService, Clock {
	...
	JWTTokenService(@Value("${jwt.issuer:mpetrescue}") final String issuer,
	...
	private String newToken(final Map<String, String> attributes, final int expiresInSec) {
		...
		final Claims claims = Jwts.claims().setIssuer(issuer)
				.setIssuedAt((Date.from(now.toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant())));

		if (expiresInSec > 0) {
			final ZonedDateTime expiresAt = now.plusSeconds(expiresInSec);
			claims.setExpiration(Date.from(expiresAt.toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
		}
	...
	public Date now() {
		final ZonedDateTime now = ZonedDateTime.now();
		return Date.from(now.toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant());
	}
